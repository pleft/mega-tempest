...
# (c_name, sprite_label, verts_label_or_None, rotation_rad, size_tiles_per_side, palette_idx_1_to_15)
...
SPRITES = [
    ("flipper_s", "s_flipper", "fverts",     0.0, 1, PAL_RED),
    ("flipper_m", "s_flipper", "fverts",     0.0, 2, PAL_RED),
    ("shot",      "pshot",     "pshotverts", 0.0, 1, PAL_WHITE),
] + [
    ...
    (f"player_l{i:02d}", "sclaw4", None, -i * math.pi / 8, 2, PAL_YELLOW)
    for i in range(16)
]
...
def extract(text, label, verts_label, angle, size_tiles, color):
    block = find_label_block(text, label)
...
def main():
    ...
    blobs = []
    for name, label, vlabel, ang, sz, col in SPRITES:
        data = extract(text, label, vlabel, ang, sz, col)
        blobs.append((name, sz, col, data))
        print(f"  {name:10s} ← {label:12s} size {sz}x{sz}  pal {col}  ({len(data)} bytes)")
