...
// Mode 1 cart adapter: instead of a 1M Word RAM scratch buffer at sub
// $0C0000 and libc malloc/free, the cart-side spx provides a static
// sample-temp buffer and a static pattern-buffer allocator. Keeps
// module.c portable while replacing the only two hardware/libc deps it
// has (Data_p init + the pattern buffer alloc/free).
extern uint8_t * const mod_sample_buf;
extern uint32_t *     mod_alloc_pattern_buf(uint32_t size_bytes);
extern void           mod_free_pattern_buf(uint32_t * buf);

// Used by Random tremolo/vibrato waveforms — implemented in spx.c.
extern int rand(void);

// --- Streaming hooks into spx.c (per-channel rings, multiple streamed
//     samples per MOD).
//
// Each of the 4 MOD channels has its own 4 KB ring in PCM RAM at
// blocks $C0-$CF, $D0-$DF, $E0-$EF, $F0-$FF. Any sample marked
// `g_sample_is_streamed[i]` is NOT loaded resident — its bytes live in
// cart ROM at `g_sample_src_offset[i]`. When such a sample triggers on
// a channel, spx pre-fills that channel's ring and refills it per tick.
//
// Limits: up to 4 streamed samples can play concurrently (one per
// channel). If a channel switches samples mid-play, the new sample
// re-uses that channel's ring.
extern uint8_t  g_sample_is_streamed[31];
extern uint32_t g_sample_src_offset [31];
extern void     mod_stream_setup       (uint8_t channel, uint8_t sample, uint32_t src_len);
extern void     mod_stream_set_period  (uint8_t channel, uint32_t period);
extern void     mod_stream_channel_stop(uint8_t channel);
extern void     mod_stream_reset_all   (void);
extern uint8_t  mod_stream_ring_block_for_channel(uint8_t channel);

// Debug probe — writes a checkpoint code to comflags_sub so main can see
// how far InitMOD got. Cheap (single byte write to a memory-mapped reg).
#define DBG(v) (*((volatile uint8_t *) 0xFF800F) = (v))
...
/* HANDLE_PANNING disabled: Tempest MODs never use Effect 8 (positional
 * pan), so PanValue stays 0 for every channel → all channels emit on a
 * single side. With this undef the player falls back to the Amiga
 * ProTracker default channel split (1,4 left / 2,3 right), giving us
 * real stereo. */
...
uint8_t InitMOD(CDFileHandle_t *File_p, Mod_t* Mod_p, uint8_t filter)
{
  ...
  uint8_t *Data_p = mod_sample_buf; /* see mod_sample_buf decl above */
  ...
  DBG(0x54);
  uint32_t pat_size = (uint32_t) Mod_p->NumberOfPatterns
                    * (uint32_t) MOD_ROWS_PER_CHANNEL
                    * (uint32_t) Mod_p->NumberOfChannels * 4u;
  DBG(0x5A);
  uint32_t * pat = mod_alloc_pattern_buf(pat_size);
  DBG(0x5B);
  Mod_p->PatternsBuff_p = pat;
  DBG(0x5C);
  if (!pat) return 3;
  DBG(0x55);
  ...
  {
    mod_free_pattern_buf(Mod_p->PatternsBuff_p);
    return 4; /* couldn't read pattern data */
  }
  DBG(0x56);

#ifdef PROC_PATTERNS
  {
    uint8_t Pattern, Row, Chan;
    for (Pattern = 0; Pattern < Mod_p->NumberOfPatterns; Pattern++)
    {
      for (Row = 0; Row < MOD_ROWS_PER_CHANNEL; Row++)
      {
        uint32_t *Div_p = &Mod_p->PatternsBuff_p[ Pattern * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels + Row * Mod_p->NumberOfChannels ];

        for ( Chan = 0; Chan < Mod_p->NumberOfChannels; Chan++ )
        {
          uint32_t division;
          ...
          */
          PeriodFrequency = ((division >> 16) & 0x0FFF);

          /* look up table here */
          for ( Cnt = 0; Cnt < MOD_NUMBER_OF_NOTES; Cnt++ )
          {
            if ( AmigaPeriodsTable[ 0 ][ Cnt ] == PeriodFrequency )
            {
              PeriodFrequency = Cnt + 4060;
              /* insert new period */
              division &= 0xF000FFFF;
              division |= PeriodFrequency << 16;
              Div_p[ Chan ] = division;
              break;
            }
          }
  ...
  /* Iteratively mark samples for streaming until the non-streamed total
   * fits in 48 KB of PCM RAM (= 64 KB - 4×4 KB rings). Largest-first.
   * Each streamed sample saves its full length in resident PCM RAM at
   * the cost of an additional 4 KB ring — but those 4 rings are already
   * reserved regardless, so streaming more samples is "free" once the
  ...
  mod_stream_reset_all();
  {
    ...
    while (1) {
      ...
      for (Sample = 0; Sample < MOD_NUMBER_OF_SAMPLES; Sample++) {
        ...
        if (Mod_p->Inst[Sample].SampleLength == 0) continue;
        total += (Mod_p->Inst[Sample].SampleLength + 32 + 255) & ~255;
      }
      if (total <= RESIDENT_BUDGET) break;
      /* Need to stream more — find largest non-streamed sample. */
      Iy = 0xFFFFFFFF;
      uint32_t biggest = 4 * 1024;   /* don't bother streaming small */
      for (Sample = 0; Sample < MOD_NUMBER_OF_SAMPLES; Sample++) {
        if (g_sample_is_streamed[Sample]) continue;
        if (Mod_p->Inst[Sample].SampleLength > biggest) {
          Iy = Sample;
          biggest = Mod_p->Inst[Sample].SampleLength;
        }
      }
      if (Iy == 0xFFFFFFFF) break;   /* nothing left we can stream */
      g_sample_is_streamed[Iy] = 1;
      /* SampleHandle for streamed samples is a sentinel (0xFE) — SetSample
       * will substitute the channel's ring block at trigger time. */
      Mod_p->Inst[Iy].SampleHandle = 0xFE;
      Mod_p->Inst[Iy].Factor = 0;
    }
  }

  /* Upstream halving-with-pitch-compensation — applies only to non-streamed
   * samples. With multi-stream, in practice rave4/tune5/tune12/tune13 etc.
   * should reach the 48 KB resident budget WITHOUT halving anything. */
  /* The streamed sample is excluded from the fit calc; PCM RAM budget for
   * the remaining samples is MOD_PCM_RAM (64 KB) - 4 KB (ring) = 60 KB. */
  Ix = Iz = 0;
  do
  {
    if (Iz > (MOD_PCM_RAM - 0xA000))
    {
      ...
      {
        if (g_sample_is_streamed[Sample]) continue;
        if ((Mod_p->Inst[ Sample ].SampleLength >> Mod_p->Inst[ Sample ].Factor) <= (1024 - 32))
          continue;
        if (Mod_p->Inst[ Sample ].Factor != Ix)
          continue;
  ...
  {
    /* Streamed sample: don't upload, just record its file offset and skip
     * past its data block. spx pulls bytes from the cart-side MOD copy
     * via this offset during playback. */
    if (g_sample_is_streamed[Sample]) {
      g_sample_src_offset[Sample] = (uint32_t) File_p->Tell(File_p);
      File_p->Seek(File_p, (int32_t) Mod_p->Inst[Sample].SampleLength, 1 /* SEEK_CUR */);
      continue;
    }
    if ( Mod_p->Inst[ Sample ].SampleLength > 0 )
    {
      ...
      if (Mod_p->Inst[ Sample ].Factor)
      {
        ...
        if (filter == 1)
        {
          /* filter coeff = .25, .5, .25 */
          for (Ix = 0; Ix < Mod_p->Inst[ Sample ].Factor; Ix++)
          {
            int16_t s = 0, a = 0;
            Iy = Iz = 0;
            Soffset = Mod_p->Inst[ Sample ].SampleLength >> Ix;
            while (Soffset <= Mod_p->Inst[ Sample ].SampleLength)
            {
              s = a + ((int8_t)Data_p[Iy++] >> 1);
              a = (int8_t)Data_p[Iy++] >> 2;
              Data_p[Iz++] = s + a;
              Soffset -= 2;
            }
          }
        ...
        {
          /* regular downsample by N (running average) */
          Iy = Iz = 0;
  ...
  Mod_p->NextBlock = Iw; /* save last returned next pcm block value */

  /* Pre-convert streamed-sample bytes in-place to RF5C164 sign-magnitude
   * format. Done once here so the hot streaming path (stream_write_chunk
   * called every 50 Hz tick + on every note retrigger) can use the cheap
   * pcm_cpy(conv=0) path — eliminates ~4 instructions per byte on the
   * sub 68k. Positive bytes are clamped to 126 specifically so the
   * converted result never produces $FF (the chip's end-of-sample
   * marker). The MOD file bytes at $60000 are writable PRG-RAM, so
   * the in-place rewrite is safe. */
  DBG(0x5A);
  for (Sample = 0; Sample < MOD_NUMBER_OF_SAMPLES; Sample++)
  {
    if (!g_sample_is_streamed[Sample]) continue;
    if (Mod_p->Inst[Sample].SampleLength == 0) continue;
    uint8_t * p   = (uint8_t *) (0x60000 + g_sample_src_offset[Sample]);
    uint32_t  len = Mod_p->Inst[Sample].SampleLength;
    while (len--)
    {
      int16_t s = (int8_t) *p;
      if (s < 0) { s = -s; if (s > 127) s = 127; }
      else       { if (s > 126) s = 126; s |= 128; }
      *p++ = (uint8_t) s;
    }
  }
  DBG(0x5B);
...
uint8_t JumpMOD(Mod_t* Mod_p, uint8_t order)
{
  ...
  uint8_t rv = Mod_p->Order;

  Mod_p->Tick = Mod_p->Speed;  /* force immediate processing */
  Mod_p->Order = order;
  Mod_p->Row = 0;
...
uint8_t TimeScaleMOD(Mod_t* Mod_p, uint8_t scale)
{
  ...
  uint8_t rv = Mod_p->TimeScale;

  Mod_p->TimeScale = scale < 0x40 ? scale : 0x3F;
...
#ifdef PROC_PATTERNS
    if ( Mod_p->Notes[ Channel ].PeriodFrequency >= 4060 )
    {
      Mod_p->Notes[ Channel ].NoteExists = TRUE;
      Mod_p->Notes[ Channel ].PeriodFrequencyCol = Mod_p->Notes[ Channel ].PeriodFrequency - 4060;
      Mod_p->Notes[ Channel ].PeriodFrequency = AmigaPeriodsTable[ 0 ][ Mod_p->Notes[ Channel ].PeriodFrequencyCol ];
    }
#else
    /* look up table here */
    for ( Cnt = 0; Cnt < MOD_NUMBER_OF_NOTES; Cnt++ )
...
static void SetSample(uint16_t Channel, uint8_t Sample)
{
  ...
  if (length == 0)
  {
    ...
    mod_stream_channel_stop(Channel);
    return;
  }
  ...
  /* Streaming hook: if this is the streamed sample, set up the channel
   * to play from the ring; otherwise notify spx that the channel is no
   * longer streaming. */
  if (g_sample_is_streamed[Sample]) {
    /* Streamed sample: route the channel through its own 4 KB ring.
     *   start  = channel's ring block ($C0..$F0)
     *   offset = 0
     *   loop   = 0 (loop_start register = ring_block<<8)
     * Spx writes a $FF terminator at the ring's last byte so the chip
     * loops there cleanly back to the ring start. */
    voices[Channel].start  = mod_stream_ring_block_for_channel((uint8_t) Channel);
    voices[Channel].offset = 0;
    voices[Channel].loop   = 0;
    mod_stream_setup((uint8_t) Channel, Sample, gMod->Inst[Sample].SampleLength);
  } else {
    mod_stream_channel_stop((uint8_t) Channel);
  }
}
...
static void SetFrequency(uint16_t Channel, uint32_t Period)
{
  // Read the halving factor from the canonical source, not from
  // voices[Channel].factor — UpdateNote calls SetFrequency BEFORE
  // SetSample when a row has both a new note and a sample change, so
  // voices[Channel].factor still holds the *previous* sample's factor.
  // Using it shifts the period by the wrong amount → samples halved
  // more aggressively than their predecessors play one or two octaves
  // higher than intended (drums sound like squeaks).
  uint8_t factor = gMod->Inst[ gMod->Channels[ Channel ].LastInstrument ].Factor;
  voices[Channel].period = Period << factor;
  voices[Channel].pending |= PENDING_PER;
  /* Tell the streaming worker the new period so it can pace its ring-fill
   * rate to match the chip's read rate (FD = 223152 / (period+1)). */
  mod_stream_set_period((uint8_t) Channel, voices[Channel].period);
}
...
static void callback(void)
{
  ...
  /* Tick counter visible to main via comstat3 ($FF8026). Increments every
   * timer interrupt, so if main sees this advance we know the INT3 chain
   * (vector $6C → JMP at $5F82 → timer_int → callback) is alive. */
  *((volatile uint16_t *) 0xFF8026) += 1;

  /* Streaming worker — refill the ring buffer for the streamed sample. */
  {
    extern void mod_stream_tick(void);
    mod_stream_tick();
  }
